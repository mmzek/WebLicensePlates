"use client";
import { getVoivodeships } from "@/actions/voivodeships";
  ...
  const shouldShowPlaceholder = !voivodeship || isLoading || isAnimating;
    return(
        <div className="w-3/4 h-full overflow-y-auto p-6 bg-background command-scroll">
            {(shouldShowPlaceholder|| voivodeship == null || voivodeship=="") && (<div className="flex h-full w-full items-center justify-center">
          <img 
    ...
    >
      <div className="flex justify-between items-start mb-6">
        ...
        <div className="bg-white/5 px-3 py-1 rounded-full border border-white/10">
          <span className="text-[10px] text-white/50 uppercase">👁 {pl.numberOfViews}</span>
        </div>
      </div>
      <div className="grid grid-cols-2 gap-4 border-t border-white/5 pt-4">
        <div>
          <p className="text-[10px] uppercase text-white/30 font-semibold">Województwo</p>
          <p className="text-sm text-white/90">{pl.voivodeship}</p>
        </div>
        <div>
          <p className="text-[10px] uppercase text-white/30 font-semibold">Powiat</p>
          <p className="text-sm capitalize text-white/90">{pl.district}</p>
        </div>
        <div className="col-span-2">
          <p className="text-[10px] uppercase text-white/30 font-semibold">Typ rejestracji</p>
